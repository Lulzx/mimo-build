...
pub fn summarize(name: &str, args: &Value) -> String {
    match name {
        ...
        "get_command_or_subagent_output" => format!("Output {}", args["task_id"].as_str().unwrap_or("?")),
        "wait_commands_or_subagents" => "Wait for background tasks".to_string(),
        "kill_command_or_subagent" => format!("Kill {}", args["task_id"].as_str().unwrap_or("?")),
        "memory_search" => format!("Recall {}", args["query"].as_str().unwrap_or("?")),
        "memory_get" => format!("Recall {}", args["name"].as_str().unwrap_or("?")),
        "update_goal" => "Update goal".to_string(),
        "scheduler_create" => format!("Schedule {}", args["command"].as_str().unwrap_or("?")),
        "scheduler_delete" => format!("Unschedule {}", args["id"].as_str().unwrap_or("?")),
        "scheduler_list" => "List schedules".to_string(),
        "monitor" => format!("Monitor {}", args["command"].as_str().unwrap_or("?")),
        "image_gen" | "image_edit" => format!("Image {}", args["prompt"].as_str().unwrap_or("?")),
        "video_gen" => format!("Video {}", args["prompt"].as_str().unwrap_or("?")),
